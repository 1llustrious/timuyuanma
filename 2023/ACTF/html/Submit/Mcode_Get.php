...
if($srow['status']==1){
	...
	if($srow['type']=='alipay'){
		...
		$QR_row['qr_url']='alipays://platformapi/startapp?appId=09999988&actionType=toAccount&goBack=NO&amount='.$srow['price'].'&userId='.$userId.'&memo=';
	}
	$data=array("qrcode"=>"https://qun.qq.com/qrcode/index?data=".urlencode($QR_row['qr_url']),"money"=>$srow['price']);
	...
	$result=array("code"=>-1,"msg"=>$_SESSION['data_msg'],"data"=>$data);
		...
		*/
		...
		if($arr['price']>0.00 or $arr['code']==1){
			...
			$price = '订单作废';
		}elseif($arr['code']==-1 and $arr['price']==-0.02){
		...
		if($price>0.00){
			...
			if(strstr($QR_row['qr_url'], 'User/QRCODE')){
				...
				if($srow['type']=='alipay'){
					$userId = getSubstr(base64_decode($QR_row['cookie']), "CLUB_ALIPAY_COM=", ";");//支付宝合作者ID
					//$QR_row['qr_url']='alipays://platformapi/startapp?appId=20000123&actionType=scan&biz_data='.urlencode('{"s": "money","u": "'.$userId.'","a": "'.$srow['price'].'","m":""}');
					$QR_row['qr_url']='alipays://platformapi/startapp?appId=09999988&actionType=toAccount&goBack=NO&amount='.$price.'&userId='.$userId.'&memo=';
				}
				$data=array("qrcode"=>"https://qun.qq.com/qrcode/index?data=".urlencode($QR_row['qr_url']),"money"=>$price);
			}
			$result=array("code"=>100,"msg"=>'请扫码支付',"data"=>$data);
		}else{
