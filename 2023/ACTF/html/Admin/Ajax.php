...
if($act=='Login'){//登录后台
    ...
    $beizhu= $_POST['beizhu'];
	$sort= $_POST['sort'];
	$is=$DB->query("SELECT * FROM `pay_wechat_trumpet` order by id desc limit 1")->fetch();
	if(!$is){
	...
	if($sds){
		...
		$result=array("code"=>-1,"msg"=>"你已经添加了微信免挂店员");
	}
	...
	if($status==5){
		...
		$is=$DB->query("SELECT * FROM `pay_wechat_trumpet` order by id desc limit 1")->fetch();
		if($DB->exec("update `pay_qrlist` set `status`='{$status}',`cookie`='Login_Yes',`wx_name`='{$is['wx_name']}',`endtime`='{$date}' where `id`='{$id}' limit 1")!==false)
			exit('{"code":200}');
		else
			exit('{"code":400,"msg":"修改失败！['.$DB->error().']"}');
	}else{
