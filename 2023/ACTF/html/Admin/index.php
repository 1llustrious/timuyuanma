...
if($_COOKIE['Query_row']!='Zero_Yas'){
$Query_row = $Instant_Api->Query();
	$http_runtime = http_runtime($conf['Instant_url']);
	$_SESSION['http_runtime'] = $http_runtime;
	
	if(!$Query_row['status'] or $Query_row['status']==0){//检测云端是否配置正确
		setcookie("Query_row", 'Zero_No', time() + 1800);
		exit("<script language='javascript'>alert('云端未成功配置或已被禁封，点击[确定]继续配置云端!');window.location.href='./Pay.php';</script>");
	}
}
...
<p>以下网址可以设置2-6秒的监控速度,若不监控,云端会帮忙监控,但是云端一旦出错,支付宝、QQ钱包cookie得不到刷新容易掉线。</p>
<p>QQ、支付宝监控以下网址(微信无需监控)</p>
<li class="list-group-item">http://<?php echo $_SERVER['HTTP_HOST']?>/Cron.php?key=<?php echo $conf['cronkey']; ?></li>
<br>
      ...
      <div class="panel panel-default">
		...
		<div class="form-group">
		  ...
		  <div class="input-group"><div class="input-group-addon">云端响应速度</div>
			<pre class="form-control"><?php echo '['.$checkServerSatatus['Country'].'] (ms:'.$checkServerSatatus['Ping'].')';?></pre>
	    </div></div>
		</li>
            <li class="list-group-item">
...
1.每笔交易会有手续费<br/>2.请确保结算信息，否则资金可能无法成功到账！<br/>3.系统集成，问题反馈请联系客服
	  ...
	  <div class="panel panel-primary">
		...
		<!--li class="list-group-item">
			<b>服务器真实IP：</b><?php echo get_curl($conf['Instant_url'].'api/instant_api.php?do=Get_Ip'); ?>
		</li-->
	</ul>
		</div>
      </div>
